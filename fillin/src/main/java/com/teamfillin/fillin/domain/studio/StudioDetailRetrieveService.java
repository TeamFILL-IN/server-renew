...
public class StudioDetailRetrieveService {
	...
	public StudioDetailResult retrieveDetail(long studioNo) {
		Studio studio = studioRetriever.retrieveOne(studioNo);
		...
		studio.setRunningStatus(runningTimeView.getCurrentOperationStatus(LocalTime.now(koreaTimeClock)));
		return new StudioDetailResult(studio, priceView, runningTimeView);
	}
}
